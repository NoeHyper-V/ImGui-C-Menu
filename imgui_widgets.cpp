...
bool ImGui::Keybind(const char* label, int* key, int* mode, bool label_active)
{
    ...
    PushStyleColor(ImGuiCol_Text, c::accent);
    ...
    PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(12, 12));
    ...
    PushStyleColor(ImGuiCol_WindowBg, c::combo::background);
...
bool ImGui::CustomColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
...
bool ImGui::CustomColorRGB(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col, float rotation_speed, bool renderInterface /* = false */)
{
    ...
    if (!renderInterface)
    {
        // Lógica de rotación y cambio de color sin renderizar la interfaz
        float time = ImGui::GetTime();
        float H = fmodf(time * rotation_speed * 0.1f, 1.0f);
        bool value_changed = true;

        // Calcula los valores de S y V en función del tiempo si la bandera NoAlpha no está presente
        if (!(flags & ImGuiColorEditFlags_NoAlpha))
        {
            float t = fmodf(time * 0.05f, 1.0f); // Modifica la velocidad de cambio ajustando el valor 0.05f
            float S = ImSaturate(0.5f + 0.5f * sinf(2 * IM_PI * t));
            float V = ImSaturate(0.5f + 0.5f * cosf(2 * IM_PI * t));
            value_changed = true;
        }

        // Convertir de nuevo el color a RGB si ha habido cambios en H, S o V
        if (value_changed)
        {
            if (flags & ImGuiColorEditFlags_InputRGB)
            {
    ...
    float sv_picker_size = ImMax(bars_width * 0.8f, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x));
    float bar0_pos_x = picker_pos.x + sv_picker_size + style.ItemInnerSpacing.x;
    float bar1_pos_x = bar0_pos_x + bars_width + style.ItemInnerSpacing.x;
    float bars_triangles_half_sz = IM_TRUNC(bars_width * 0.20f);
    float backup_initial_col[4];
    memcpy(backup_initial_col, col, components * sizeof(float));
    float wheel_thickness = sv_picker_size * 0.05f;
    float wheel_r_outer = sv_picker_size * 0.3f;
    float wheel_r_inner = wheel_r_outer - wheel_thickness;
    ImVec2 wheel_center(picker_pos.x + (sv_picker_size + bars_width) * 0.5f, picker_pos.y + sv_picker_size * 0.5f);
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        // Hue wheel + SV triangle logic
        float time = ImGui::GetTime();
        H = fmodf(time * rotation_speed * 0.1f, 1.0f);
        value_changed = value_changed_h = true;

        // Calcula los valores de S y V en función del tiempo
        if (!(flags & ImGuiColorEditFlags_NoAlpha))
        {
            float t = fmodf(time * 0.05f, 1.0f); // Modifica la velocidad de cambio ajustando el valor 0.05f
            S = ImSaturate(0.5f + 0.5f * sinf(2 * IM_PI * t));
            V = ImSaturate(0.5f + 0.5f * cosf(2 * IM_PI * t));
            value_changed = value_changed_sv = true;
        }
    }
    // Alpha bar logic
...
void ImGui::GlowText(ImFont* font, float size, const char* label, ImVec2 pos, ImU32 text_color)
{
...
bool ImGui::Tabs(bool selected, ImTextureID image, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    PushStyleVar(ImGuiStyleVar_Alpha, it_anim->second.background_opticaly);
    ...
    GlowText(font::inter_bold, 17.f, label, bb.Min + ImVec2(50, (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text));
...
bool ImGui::SubTab(bool selectable, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    PushStyleVar(ImGuiStyleVar_Alpha, it_anim->second.background_opticaly);
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(size_arg.x / 3, size_arg.y - 4), bb.Max - ImVec2(size_arg.x / 4, 0), GetColorU32(c::accent), 30.f, ImDrawCornerFlags_Top);
    //  ImGui::GetWindowDrawList()->AddShadowRect(bb.Min + ImVec2(0, size_arg.y - 1), bb.Max, 25.f, ImVec2(0, -2), GetColorU32(c::accent), 0.f);
