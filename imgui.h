...
namespace ImGui
{
    ...
    //    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding
    //    BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
    IMGUI_API bool          CustomBeginChild(const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags flags = 0);
    IMGUI_API bool          CustomBeginChild(ImGuiID id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags flags = 0);
    IMGUI_API void          CustomEndChild();
    ...
    IMGUI_API void          GlowText(ImFont* font, float size, const char* label, ImVec2 pos, ImU32 text_color);
    ...
    IMGUI_API bool          CustomColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags = 0, const float* ref_col = NULL);

    IMGUI_API bool CustomColorRGB(const char* label, float col[4], ImGuiColorEditFlags flags = 0, const float* ref_col = NULL, float rotation_speed = 1.0f, bool renderInterface = false);
    ...
    IMGUI_API bool          CustomBeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags flags = 0); // helper to create a child window / scrolling region that looks like a normal widget frame
    IMGUI_API void          CustomEndChildFrame();                                                    // always call EndChildFrame() regardless of BeginChildFrame() return values (which indicates a collapsed/clipped window)
